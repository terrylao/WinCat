...
void WinCat::setUdp(char * restrictip) {
     udpsocket =1;
...
int WinCat::Process(SOCKET ClientSocket) {
    ...
    for(;;) {
        ...
        if (FD_ISSET(ClientSocket, &readfds)) {
            ...
            if (udpsocket==1){
                iResult = recvfrom(ClientSocket, recvbuf, DEFAULT_BUFLEN,0, (SOCKADDR *)&SenderAddr, &SenderAddrSize);
            }else{
            ...
            if (bytes_read > 0) {
                if (udpsocket==1){
                    iResult = sendto(ClientSocket, sendbuf, bytes_read, 0, (struct sockaddr*) &SenderAddr, SenderAddrSize);
                }else{
